...
test('pendant l’essai, la prochaine facture est la fin de l’essai', () => {
  const r = ecritureDepuisEvenement({
    type: 'customer.subscription.updated', created: 1,
    data: { object: { id: 'sub_1', status: 'trialing', trial_end: 1_760_000_000,
                      metadata: { garage_id: GARAGE, offre: 'atelier' } } },
  })
  assert.equal(r.champs.abonnement_prochaine_facture, new Date(1_760_000_000_000).toISOString())
})

test('hors essai, on lit la fin de période, au niveau ligne ou racine', () => {
  const ligne = ecritureDepuisEvenement({
    type: 'customer.subscription.updated', created: 1,
    data: { object: { id: 'sub_1', status: 'active',
                      items: { data: [{ current_period_end: 1_770_000_000 }] },
                      metadata: { garage_id: GARAGE } } },
  })
  assert.equal(ligne.champs.abonnement_prochaine_facture, new Date(1_770_000_000_000).toISOString())
  const racine = ecritureDepuisEvenement({
    type: 'customer.subscription.updated', created: 1,
    data: { object: { id: 'sub_1', status: 'active', current_period_end: 1_780_000_000,
                      metadata: { garage_id: GARAGE } } },
  })
  assert.equal(racine.champs.abonnement_prochaine_facture, new Date(1_780_000_000_000).toISOString())
})

test('un abonnement fermé n’annonce plus aucun prélèvement', () => {
  const r = ecritureDepuisEvenement({
    type: 'customer.subscription.deleted', created: 1,
    data: { object: { id: 'sub_1', trial_end: 1_760_000_000, metadata: { garage_id: GARAGE } } },
  })
  assert.equal(r.champs.abonnement_prochaine_facture, null)
})

test('une date absente ou aberrante ne fabrique pas de fausse échéance', () => {
  for (const objet of [{ id: 's', status: 'active' }, { id: 's', status: 'active', trial_end: 0 },
                       { id: 's', status: 'active', trial_end: 'demain' }]) {
    const r = ecritureDepuisEvenement({ type: 'customer.subscription.updated', created: 1,
      data: { object: { ...objet, metadata: { garage_id: GARAGE } } } })
    assert.equal(r.champs.abonnement_prochaine_facture, null)
  }
})
