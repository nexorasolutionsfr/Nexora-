 ...
 *
 * Pendant l'essai, c'est `trial_end` : la date du PREMIER prélèvement, celle
 * qui compte pour le garage et dont il doit être averti.
 *
 * Ensuite, c'est la fin de période courante. Stripe l'a déplacée au niveau de
 * la LIGNE d'abonnement dans ses versions récentes, mais la garde à la racine
 * dans les anciennes — on lit les deux plutôt que de parier sur une version
 * d'API qui changera sans nous prévenir.
 */
function prochaineFacture(abonnement) {
  const secondes =
    abonnement?.trial_end ??
    abonnement?.items?.data?.[0]?.current_period_end ??
    abonnement?.current_period_end;
  if (typeof secondes !== "number" || secondes <= 0) return null;
 ...
 *
 * `gardeOrdre` dit si l'écriture doit être protégée contre le désordre de
 * livraison. Seuls les événements qui portent un STATUT le sont.
 *
 * DÉFAUT CORRIGÉ, TROUVÉ EN RECETTE RÉELLE
 *
 * La garde s'appliquait à tous les événements, y compris au rattachement de
 * session. Or Stripe crée l'abonnement et clôt la session de paiement dans la
 * même seconde, et livre `checkout.session.completed` en premier. Le
 * rattachement posait donc `abonnement_maj_le`, puis
 * `customer.subscription.created`, daté de la même seconde ou d'avant, était
 * rejeté comme « événement dépassé ».
 *
 * Résultat observé sur un vrai paiement : le garage était bien lié à Stripe —
 * client, abonnement, forfait — mais `abonnement_actif` restait faux et
 * l'accès ne s'ouvrait pas. Le pire des cas : quelqu'un paie et reste dehors,
 * sans erreur nulle part, parce que la route répond 200 et que Stripe ne
 * rejoue donc rien.
 *
 * Le rattachement n'écrit aucun statut : il ne peut rien faire régresser, et
 * n'a donc pas besoin d'être protégé. Il ne doit surtout pas bloquer ce qui
 * suit.
 *
...
export function ecritureDepuisEvenement(evenement) {
  ...
  if (type === "checkout.session.completed") {
    ...
    return { cible: { colonne: "id", valeur: garageId }, champs, evenementLe, ecraserLesNuls: false, gardeOrdre: false };
  }
  ...
  if (
    ...
    const champs = {
      ...
      // Le motif d'accès suit l'abonnement, mais dans un seul sens : on l'écrit
      // à l'ouverture, on n'y touche pas à la fermeture. Un abonnement résilié
      // laisse `acces_motif = 'abonnement'` avec `abonnement_actif = false`,
      // ce qui se lit encore six mois plus tard : « il a été abonné, il ne
      // l'est plus ». Écraser le motif effacerait cette histoire.
      ...(ouvert ? { acces_motif: "abonnement", acces_fin: null } : {}),
      ...
      // Un abonnement fermé n'a plus de prochaine facture : laisser une date
      // ferait afficher au garage un prélèvement qui n'aura jamais lieu.
      abonnement_prochaine_facture: ouvert ? prochaineFacture(objet) : null,
    };
    return { cible, champs, evenementLe, ecraserLesNuls: true, gardeOrdre: true };
  }
