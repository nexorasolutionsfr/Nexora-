...
function regleCritair(vehicule) {
  ...
  return {
    ...
    faitsUtilises: faits,
    // Proposée, jamais exigée : elle ne s'affiche que lorsque le classement
    // repose sur le repli, donc là où elle change vraiment quelque chose.
    precision: r.precision === "norme_euro" ? PRECISIONS.norme_euro : null,
    limites: r.limites,
    liens: [
...
function regleCampagnes(vehicule, campagnes) {
  ...
  // Le détail ne se dit que s'il apprend quelque chose : préciser « 1 nomme
  // exactement ce modèle » quand il n'y en a qu'une est du bruit.
  const detail =
    r.voisines === 0
      ? null
      : r.exactes === 0
        ? `toutes nomment une version voisine`
        : `${r.exactes} exactement ce modèle, ${r.voisines} une version voisine`;
  ...
  return {
    ...
    resume:
      nombre === 0
        ...
        : `${nombre} fiche${nombre > 1 ? "s" : ""} publiée${nombre > 1 ? "s" : ""} nomme${nombre > 1 ? "nt" : ""} ce modèle${detail ? ` (${detail})` : ""}. Nexora ne peut pas savoir si VOTRE voiture est concernée : cela se vérifie avec son numéro de série.`,
    valeur: {
      retenues: r.retenues,
      ecartees: r.ecartees,
      total: r.total,
...
// La seule vérification qui tranche vraiment. Nexora ne prétend pas connaître
// l'adresse du service de chaque marque : elle nomme le geste, l'endroit où
// trouver le numéro, et laisse la fiche officielle porter le code de campagne
// dont le réseau a besoin.
function VERIFICATION_VIN(vehicule) {
  return {
    titre: "Vérifier pour VOTRE voiture",
    texte: `Votre numéro de série (VIN) est en case E de la carte grise. Avec lui, le réseau ${vehicule.marque} dit en une question si votre voiture est concernée — c'est la seule réponse qui vaut pour elle.`,
    lien: { libelle: "Toutes les fiches officielles sur rappel.conso.gouv.fr", url: "https://rappel.conso.gouv.fr/" },
  };
}
...
export const PROGRAMMES = [
  {
    ...
    reserves: [
      "Tesla introduit sa liste par « s'ils s'appliquent à votre véhicule » : c'est à vous de reconnaître ce qui concerne la vôtre.",
      ...
      "La condition « fabriqués avant 2021 environ » porte sur la date de FABRICATION. Nexora ne connaît que votre date d'immatriculation, et « environ » n'est pas une date : elle affiche la condition sans l'appliquer.",
    ],
    operations: [
      { libelle: "Liquide de frein : contrôle (remplacement au besoin)", intervalle: { mois: 48 }, condition: "Remorquage, descentes de montagne, conduite sportive, environnement chaud et humide : contrôles plus fréquents." },
      { libelle: "Filtre à air d'habitacle : remplacement", intervalle: { mois: 24 } },
      { libelle: "Balais d'essuie-glace : remplacement", intervalle: { mois: 12 } },
      { libelle: "Étriers de frein : nettoyage et graissage", intervalle: { mois: 12, km: 20000 }, condition: "Uniquement dans les régions où les routes sont salées en hiver." },
      { libelle: "Permutation des pneus", intervalle: { km: 10000 }, condition: "Ou dès 1,5 mm d'écart de profondeur entre les pneus, selon ce qui arrive en premier." },
      { libelle: "Sachet de déshydratant de la climatisation : remplacement", intervalle: { mois: 72 }, condition: "Véhicules fabriqués avant 2021 environ." },
    ],
  },
  {
    cle: "tesla_model_y",
    ...
    reserves: [
      "Tesla introduit sa liste par « s'ils s'appliquent à votre véhicule » : c'est à vous de reconnaître ce qui concerne la vôtre.",
      "Tesla ajoute que cette liste n'est pas exhaustive et n'inclut pas les consommables — tout en y faisant figurer les balais d'essuie-glace. Nexora affiche la liste telle qu'elle est publiée, sans trancher cette contradiction.",
      "Les filtres HEPA et à charbon dépendent de l'équipement : la page ne dit pas comment savoir si votre voiture en a.",
    ],
    operations: [
      { libelle: "Liquide de frein : contrôle (remplacement au besoin)", intervalle: { mois: 48 }, condition: "Remorquage, descentes de montagne, conduite sportive, environnement chaud et humide : contrôles plus fréquents." },
      { libelle: "Filtre à air d'habitacle : remplacement", intervalle: { mois: 24 }, condition: "Filtre HEPA et filtres à charbon, selon équipement : tous les 3 ans." },
      { libelle: "Filtre HEPA : remplacement", intervalle: { mois: 36 }, condition: "Selon équipement." },
      { libelle: "Balais d'essuie-glace : remplacement", intervalle: { mois: 12 } },
      { libelle: "Étriers de frein : nettoyage et graissage", intervalle: { mois: 12, km: 20000 }, condition: "Uniquement dans les régions où les routes sont salées en hiver." },
      { libelle: "Permutation des pneus", intervalle: { km: 10000 }, condition: "Ou dès 1,5 mm d'écart de profondeur entre les pneus, selon ce qui arrive en premier." },
    ],
...
function regleEntretien(vehicule, programmes, reperes) {
  ...
  if (programme) {
    return {
      ...
      resume: `${vehicule.marque} publie les opérations d'entretien de ce modèle et leurs intervalles.`,
      valeur: { niveau: "programme", operations: programme.operations, portee: programme.portee },
      // La phrase qui doit toujours accompagner un programme. Un intervalle
      // publié dit « tous les 2 ans » ; il ne dit pas « le 4 mars prochain ».
      // Sans la date de la dernière fois, il n'y a pas d'échéance, et laisser
      // croire le contraire serait inventer un calendrier.
      avertissement:
        "C'est le programme publié par le constructeur, pas votre calendrier. Aucune de ces dates n'est calculée pour votre voiture tant que vous n'avez pas enregistré la dernière fois que l'opération a été faite.",
      faitsUtilises: [{ cle: "marque_modele", valeur: `${vehicule.marque} ${vehicule.modele}` }],
