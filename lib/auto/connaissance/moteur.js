...
function regleCampagnes(vehicule, campagnes) {
  ...
  // Quatre phrases, une par groupe, dans l'ordre où l'on descend. Chacune dit
  // ce qu'elle sait ET ce qu'elle ne sait pas : une fiche « à vérifier » ne
  // devient jamais une fiche « qui vous concerne », et une fiche dont les
  // dates intriguent ne devient jamais une fiche écartée.
  const resume = (() => {
    const { principales: p, aConfirmer: a, voisines: v, ecartees: e } = r;
    if (p.length > 0) {
      return `${p.length} fiche${s(p.length)} publiée${s(p.length)} nomme${nt(p.length)} « ${r.terme} ». Nexora ne peut pas savoir si votre voiture est concernée : cela se vérifie avec son numéro de série.`;
    }
    if (a.length > 0) {
      return `${a.length} fiche${s(a.length)} nomme${nt(a.length)} « ${r.terme} », avec une période de fabrication à vérifier pour votre voiture. Seul son numéro de série tranche.`;
    }
    if (v.length > 0) {
      return `Aucune fiche ne nomme « ${r.terme} » directement. ${v.length} nomme${nt(v.length)} une version voisine.`;
    }
    return `${e.length} fiche${s(e.length)} nomme${nt(e.length)} ce modèle, mais leur fabrication a commencé après la mise en circulation de votre voiture.`;
  })();
  ...
  return {
    ...
    limites: [
      ...
      "La période affichée est une période de fabrication ou de commercialisation ; la vôtre est une date d'immatriculation. Nexora tolère douze mois d'écart, mais c'est une hypothèse : une voiture de stock ou importée s'immatricule plus tard sans cesser d'appartenir au lot. Au-delà, elle dit « période à vérifier », jamais « vous n'êtes pas concerné ».",
