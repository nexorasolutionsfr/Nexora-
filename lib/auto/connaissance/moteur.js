...
function regleCampagnes(vehicule, campagnes) {
  ...
  // Le détail ne se dit que s'il apprend quelque chose — et quand il se dit,
  // il doit s'additionner. « 10 fiches (2 exactement, 3 une version voisine) »
  // laissait cinq fiches sans explication.
  const detail = [
    r.exactes > 0 ? `${r.exactes} exactement ce modèle` : null,
    r.generations > 0 ? `${r.generations} une autre génération` : null,
    r.voisines > 0 ? `${r.voisines} une version voisine` : null,
  ].filter(Boolean);
  const detailTexte = r.voisines === 0 && r.generations === 0 ? null : detail.join(", ");
  ...
  return {
    ...
    resume:
      nombre === 0
        ...
        : `${nombre} fiche${nombre > 1 ? "s" : ""} publiée${nombre > 1 ? "s" : ""} nomme${nombre > 1 ? "nt" : ""} ce modèle${detailTexte ? ` (${detailTexte})` : ""}. Nexora ne peut pas savoir si VOTRE voiture est concernée : cela se vérifie avec son numéro de série.`,
    valeur: {
      retenues: r.retenues,
      ecartees: r.ecartees,
      total: r.total,
