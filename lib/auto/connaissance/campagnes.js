...
export function campagnesPour({ vehicule, fiches = [] } = {}) {
  const vide = { retenues: [], principales: [], voisines: [], ecartees: [], total: 0, terme: null, precision: null, exactes: 0, generations: 0 };
  ...
  // Une fiche qui nomme une version voisine n'a rien à faire au premier plan :
  // elle est rangée à part, dépliable, jamais supprimée.
  const principales = retenues.filter((c) => c.correspondance !== "voisine");
