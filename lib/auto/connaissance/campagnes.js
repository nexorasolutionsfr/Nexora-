...
export function campagnesPour({ vehicule, fiches = [] } = {}) {
  if (!vehicule?.marque || !vehicule?.modele) {
    return { etat: "donnees_insuffisantes", manques: ["marque_modele"], retenues: [], ecartees: [], total: 0 };
  }
  ...
  const candidates = fiches
    ...
    .filter((f) => mentionneModele(f?.modeles_ou_references, vehicule.modele))
    .map((f) => resumerFiche(f, vehicule))
