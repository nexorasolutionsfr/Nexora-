...
import { campagnesPour, lirePeriode, memeMarque, mentionneModele, modeleVoisin, situerDansLaPeriode, termesDeModele } from "./campagnes.js";
...
test("un libellé de voiture n'est pas un nom de catalogue", () => {
  // Relevé sur une vraie voiture de Test : « Peugeot 208 (essai) ». Chercher
  // ce libellé entier ne trouve rien, et l'écran conclurait « aucune
  // campagne » — faux, et exactement ce qu'on ne veut pas dire.
  assert.deepEqual(termesDeModele("208 (essai)"), ["208"]);
  assert.deepEqual(termesDeModele("Clio IV"), ["clio iv", "clio"]);
  assert.deepEqual(termesDeModele("C3 Picasso"), ["c3 picasso", "c3"]);
  assert.deepEqual(termesDeModele("  "), []);
});
...
test("« 208 (essai) » trouve les campagnes de la 208", () => {
  const r = campagnesPour({ vehicule: { marque: "Peugeot", modele: "208 (essai)", annee: 2021 }, fiches: [FICHE_208] });
  assert.equal(r.etat, "a_verifier");
  assert.equal(r.terme, "208");
  assert.equal(r.elargi, false, "retirer une parenthèse n'est pas élargir");
});

test("le terme le plus précis gagne, et un élargissement se dit", () => {
  ...
  assert.equal(precis.elargi, false);
  ...
  assert.equal(large.elargi, true, "aucune fiche « c3 picasso » : on élargit, et on le dit");
