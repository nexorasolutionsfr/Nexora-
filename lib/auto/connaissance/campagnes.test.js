...
test("le détail des correspondances s'additionne", () => {
  ...
  assert.equal(r.exactes + r.generations + r.voisines, r.retenues.length, "les trois niveaux couvrent toutes les fiches retenues");
  assert.deepEqual([r.exactes, r.generations, r.voisines], [1, 1, 1]);
});
