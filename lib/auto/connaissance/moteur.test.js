...
test("une classe obtenue par la date est une estimation, et le dit", () => {
  ...
  assert.ok(c.precision, "la réserve qui explique pourquoi doit être là");
});
...
test("une classe obtenue par l'énergie n'est pas une estimation", () => {
  ...
  assert.equal(c.precision, null, "rien à préciser : la date ne joue aucun rôle ici");
});

test("un programme publié n'est jamais présenté comme un calendrier personnel", () => {
  const r = connaissancesDe({ vehicule: voiture({ marque: "Tesla", modele: "Model 3" }), campagnes: lues([]), aujourdhui: "2026-09-20" });
  const p = trouver(r, "programme_entretien");
  assert.ok(p.avertissement.includes("pas votre calendrier"), p.avertissement);
  assert.ok(p.avertissement.includes("enregistré la dernière fois"));
  // Les réserves de la source elle-même sont reprises, pas gommées.
  ...
  assert.ok(p.limites.some((l) => l.includes("date de FABRICATION")), "la condition « avant 2021 environ » ne doit pas être appliquée en douce");
});
