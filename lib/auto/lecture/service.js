...
// 3. Lecture désactivée : « indisponible », la facture reste conservée et se
//    renseigne à la main. Par défaut, la lecture est GRATUITE (texte des PDF) ;
//    la lecture payante demande une activation explicite (configuration.js).
// 4. Format, taille, pages, et contenu réel du fichier (ses premiers octets,
//    pas son type annoncé) : vérifiés avant toute réservation et tout appel.
...
import { verifierContenu } from "../documents.js";
...
export async function lireFacture({
  ...
  const avant = lisibleAutomatiquement({ typeMime: document.type_mime, taille: document.taille_octets, pages: 1, formats });
  ...
  // Un fichier dont le contenu ne correspond pas au type enregistré n'est pas lu.
  if (verifierContenu(octets, document.type_mime).typeMime !== document.type_mime) return { etat: "illisible", raison: "contenu" };
  const pages = document.type_mime === "application/pdf" ? compterPagesPdf(octets) : null;
  const verification = lisibleAutomatiquement({ typeMime: document.type_mime, taille: octets.byteLength, pages, formats });
  ...
  // Lecture gratuite : aucune réserve, aucun budget ; tentatives et quota
  // s'appliquent quand même.
  const reserve = configuration.gratuit ? 0 : reserveMicroUsd({ jetonsEntree: limites.jetonsEntreeMax, jetonsSortieMax: limites.jetonsSortieMax }, configuration.modele);
  ...
  try {
    ...
    await terminer({
      statut: "reussie",
      facturation: configuration.gratuit ? "non_facturee" : "facturee",
      tokens_entree: usage?.input_tokens ?? null,
      tokens_sortie: usage?.output_tokens ?? null,
      cout_estime_micro_usd: configuration.gratuit ? 0 : coutMicroUsd(usage, fournisseur.modele),
      duree_ms: dureeMs,
    });
    ...
    // Jamais le message ni la pile : ils peuvent contenir du texte de la facture.
    if (!connue) journal.error("[lecture] erreur interne", lectureId, e?.name ?? "Erreur");
