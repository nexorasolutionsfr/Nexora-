...
test("Configuration : aucun appel payant sans clé, sans budget, ou en Production", () => {
  assert.deepEqual(configurationLecture({}), { disponible: false, raison: "cle_absente" });
  assert.deepEqual(configurationLecture({ ANTHROPIC_API_KEY: "x" }), { disponible: false, raison: "budget_absent" });
  assert.deepEqual(configurationLecture({ ANTHROPIC_API_KEY: "x", AUTO_LECTURE_BUDGET_USD: "0" }), { disponible: false, raison: "budget_absent" });
  assert.deepEqual(configurationLecture({ ANTHROPIC_API_KEY: "x", AUTO_LECTURE_BUDGET_USD: "5", VERCEL_ENV: "production" }), { disponible: false, raison: "production" });
  assert.deepEqual(
    configurationLecture({ ANTHROPIC_API_KEY: "x", AUTO_LECTURE_BUDGET_USD: "5", NEXT_PUBLIC_SUPABASE_URL: "https://omphppsmhmyllapdqevn.supabase.co" }),
    { disponible: false, raison: "production" },
  );
  assert.deepEqual(configurationLecture({ ANTHROPIC_API_KEY: "x", AUTO_LECTURE_BUDGET_USD: "5", AUTO_LECTURE_MODELE: "claude-opus-5" }), { disponible: false, raison: "modele_sans_tarif" });
  assert.deepEqual(configurationLecture({ ANTHROPIC_API_KEY: "x", AUTO_LECTURE_BUDGET_USD: "5", NEXT_PUBLIC_SUPABASE_URL: "https://slawilafseganlbghgwx.supabase.co" }), {
    disponible: true,
  ...
  const base = { ANTHROPIC_API_KEY: "x", AUTO_LECTURE_BUDGET_USD: "5" };
  assert.equal(configurationLecture({ ...base, AUTO_LECTURE_QUOTA_24H: "20" }).lecturesParCompte24h, 20);
  assert.equal(configurationLecture({ ...base, AUTO_LECTURE_QUOTA_24H: "5000" }).lecturesParCompte24h, 50);
  assert.equal(configurationLecture({ ...base, AUTO_LECTURE_QUOTA_24H: "n'importe" }).lecturesParCompte24h, 10);
});
