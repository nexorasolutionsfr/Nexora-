...
const CONFIG = { disponible: true, fournisseur: "anthropic", modele: "claude-haiku-4-5-20251001", gratuit: false, formats: ["application/pdf", "image/jpeg", "image/png", "image/webp"], budgetMicroUsd: 5000000 };
const GRATUITE = { disponible: true, fournisseur: "texte_pdf", modele: "regles-1", gratuit: true, formats: ["application/pdf"], budgetMicroUsd: null, lecturesParCompte24h: 10 };
const PDF = new TextEncoder().encode("%PDF-1.4\n<< /Type /Pages /Count 1 >> << /Type /Page >>\n%%EOF");

function faux({ document, refus = null, fournisseur, contenu = PDF }) {
  ...
  const clientPersonne = {
    ...
    storage: { from: () => ({ download: async () => ({ data: new Blob([contenu]), error: null }) }) },
  };
  const clientServeur = {
...
test("Service : lecture gratuite sans budget ni coût, photos non lues, tentatives toujours journalisées", async () => {
  const f = faux({ document, fournisseur: { lire: async () => ({ brut: { est_facture_vehicule: true, operations: [] }, usage: null, dureeMs: 40 }) } });
  const r = await lireFacture({ documentId: "d1", configuration: GRATUITE, ...f.deps });
  assert.equal(r.etat, "proposee");
  assert.equal(f.traces.reservations[0].p_cout_reserve_micro_usd, 0);
  assert.equal(f.traces.reservations[0].p_budget_micro_usd, null);
  assert.deepEqual([f.traces.journal[0].facturation, f.traces.journal[0].cout_estime_micro_usd], ["non_facturee", 0]);

  const photo = faux({ document: { ...document, type_mime: "image/jpeg" }, fournisseur: { lire: async () => assert.fail("aucune lecture attendue") } });
  assert.deepEqual(await lireFacture({ documentId: "d1", configuration: GRATUITE, ...photo.deps }), { etat: "illisible", raison: "format" });
  assert.equal(photo.traces.reservations.length, 0);
});

test("Service : un fichier dont le contenu n'est pas un PDF n'est ni réservé ni lu", async () => {
  const jamais = { lire: async () => assert.fail("aucun appel attendu") };
  const f = faux({ document, fournisseur: jamais, contenu: new TextEncoder().encode('{"statusCode":"400","error":"InvalidJWT","message":"exp claim timestamp check failed"}') });
  assert.deepEqual(await lireFacture({ documentId: "d1", configuration: GRATUITE, ...f.deps }), { etat: "illisible", raison: "contenu" });
  assert.equal(f.traces.reservations.length + f.traces.comptages + f.traces.lectures, 0);
});
