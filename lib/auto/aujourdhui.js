...
// Faut-il demander le compteur ? La règle tient en une phrase : **on ne le
// demande que s'il sert à une échéance**, et jamais tant que deux relevés se
// contredisent — dans ce cas c'est une clarification qu'il faut, pas un
// chiffre de plus.
//
// C'est la règle qu'applique déjà la fiche du véhicule (`demandeActualisation`,
// calculé par `elementRevision`) : l'accueil s'y aligne au lieu d'en tenir une
// seconde, plus grossière. Un simple délai de fraîcheur réclamait le compteur
// d'une voiture dont aucune échéance ne dépend du compteur.
//
// `estimation` est la sortie de `estimerKilometrage`.
export function sollicitationKilometrage({ elements = [], vehiculeId = null, estimation = null } = {}) {
  if (estimation?.aVerifier) return "a_verifier";
  const utile = elements.some((el) => el.vehicule?.id === vehiculeId && el.demandeActualisation);
  return utile ? "utile" : "inutile";
}
