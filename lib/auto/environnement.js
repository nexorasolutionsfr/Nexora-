...
export function verdictProjet({ navigateur, serveur } = {}, projet = PROJET_TEST) {
  ...
  if (!navigateur) ecarts.push("navigateur : contrôle non effectué");
    ...
    if (!cleConforme(navigateur.clePublique, projet, "anon")) ecarts.push(`navigateur : la clé publique n'est pas une clé de ${nom} acceptée par lui`);
  }
  if (!serveur) ecarts.push("serveur : contrôle non effectué");
    ...
    if (!cleConforme(serveur.clePublique, projet, "anon")) ecarts.push(`serveur : la clé publique n'est pas une clé de ${nom} acceptée par lui`);
    if (!cleConforme(serveur.cleService, projet, "service_role")) ecarts.push(`serveur : la clé de service n'est pas une clé de ${nom} acceptée par lui`);
