...
test("effetsEnregistrement annonce ce qui va changer, et rien d'autre", () => {
  const outils = {
    libelleType: "Révision",
    formaterDate: (d) => `2 sept. 2026 (${d})`,
    formaterMontant: (m) => `${m} €`,
    formaterKm: (k) => `${k} km`,
  };
  const vehicule = { marque: "Opel", modele: "Corsa" };

  const complet = effetsEnregistrement({ vehicule, saisie: { dateIntervention: "2026-09-02", montant: "335", kilometrage: "84 500" }, ...outils });
  assert.equal(complet.length, 4);
  assert.match(complet[0], /intervention « Révision »/);
  assert.match(complet[0], /Opel Corsa/);
  assert.match(complet[1], /335 €/);
  assert.match(complet[1], /une seule fois/);
  assert.match(complet[2], /84500 km/);
  assert.match(complet[3], /justificatif/);

  // Sans montant ni compteur, on n'annonce pas ce qui n'arrivera pas.
  const minimal = effetsEnregistrement({ vehicule, saisie: { dateIntervention: "2026-09-02" }, ...outils });
  assert.equal(minimal.length, 2);
  assert.doesNotMatch(minimal.join(" "), /dépenses|compteur/);

  // Un montant vide, nul ou illisible ne compte pas.
  for (const montant of ["", "0", "abc", null]) {
    assert.equal(effetsEnregistrement({ vehicule, saisie: { montant }, ...outils }).length, 2, `montant ${montant}`);
  }

  // Mode document : une seule phrase, et elle dit ce qui ne se passera pas.
  const document = effetsEnregistrement({ vehicule, mode: "document", ...outils });
  assert.equal(document.length, 1);
  assert.match(document[0], /Aucune intervention ni dépense/);
});
