...
import { choisirVoiture, etatAujourdhui } from "./aujourdhui.js";
...
test("un rappel reporté reste affiché mais ne reprend pas la première place", () => {
  ...
  assert.equal(etat.autres[0].titre, "Contrôle technique");
});

test("l'urgence distingue le retard, l'horizon et le plus tard", () => {
  ...
  assert.equal(avec({ niveau: "ok", tri: 240 }), "plus_tard");
});
...
test("une échéance calculée, même lointaine, n'est pas « rien de calculable »", () => {
  ...
  assert.equal(etat.principale.urgence, "plus_tard");
});
