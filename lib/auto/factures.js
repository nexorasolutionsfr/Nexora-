...
// Ce que l'écran propose d'abord. Un document que la lecture ne reconnaît pas
// comme une facture (devis, attestation…) est gardé comme simple document :
// aucune intervention ni dépense n'est préparée, sauf si la personne affirme
// que c'est bien une facture.
export function modeInitial(proposition) {
  return proposition?.estFacture === false ? "document" : "intervention";
}
...
// Ce que l'enregistrement va changer, dit avant de cliquer. Une confirmation
// qui n'annonce pas ses effets demande un acte de foi : la personne doit
// pouvoir lire, en une phrase par ligne, ce qui entrera dans son dossier.
// Pur : l'écran ne fait que l'afficher.
export function effetsEnregistrement({ vehicule, saisie = {}, mode = "intervention", libelleType, formaterDate, formaterMontant, formaterKm }) {
  const nom = vehicule ? `${vehicule.marque} ${vehicule.modele}` : "cette voiture";
  if (mode !== "intervention") return [`Le document est rangé dans les documents de votre ${nom}. Aucune intervention ni dépense n'est enregistrée.`];

  const effets = [];
  const quand = saisie.dateIntervention ? ` du ${formaterDate(saisie.dateIntervention)}` : "";
  effets.push(`Une intervention « ${libelleType} »${quand} entre dans l'historique de votre ${nom}.`);

  // Les mêmes lectures que la validation : une saisie illisible n'annonce rien.
  const euros = montant(saisie.montant);
  if (Number.isFinite(euros) && euros > 0) effets.push(`${formaterMontant(euros)} s'ajoutent à vos dépenses, comptés une seule fois.`);

  const km = entier(saisie.kilometrage);
  if (Number.isFinite(km) && km > 0) effets.push(`Le compteur est enregistré à ${formaterKm(km)}, à la date de l'intervention.`);

  effets.push("Le document est rangé et rattaché à cette intervention : il en devient le justificatif.");
  return effets;
}

// La plaque lue est-elle celle d'une AUTRE voiture du garage ? C'est le cas le
// plus trompeur : on enregistre une facture sur la voiture qu'on regardait,
// alors qu'elle porte la plaque d'une autre — et si la voiture regardée n'a pas
// de plaque, `plaqueDifferente` ne voit rien. Rend la voiture concernée, ou null.
export function voitureDeLaPlaque(vehicules, immatriculationLue, vehiculeCourantId) {
  if (!immatriculationLue || !Array.isArray(vehicules)) return null;
  const lue = normaliserImmatriculation(immatriculationLue);
  if (!lue) return null;
  return (
    vehicules.find(
      (v) => v && v.id !== vehiculeCourantId && v.immatriculation && normaliserImmatriculation(v.immatriculation) === lue,
    ) ?? null
  );
}
