...
import { PROJET_TEST, projetDepuisAdresse } from "./environnement.js";
...
// À quoi ce déploiement est-il relié ? Sert au contrôle d'une prévisualisation
// (route /api/auto/environnement) : aucune clé, aucun secret, seulement des
// informations déjà publiques (l'adresse Supabase figure dans le code servi).
// Le contrôle complet, navigateur et serveur, clés comprises : environnement.js.
export function descriptionEnvironnement(env = {}) {
  const projet = projetDepuisAdresse(env.NEXT_PUBLIC_SUPABASE_URL);
  const base = projet === PROJET_PRODUCTION ? "production" : projet === PROJET_TEST ? "test" : projet ? "autre" : "inconnue";
  return {
    environnement: env.VERCEL_ENV ?? "local",
    base,
    projetSupabase: projet,
    regionFonction: env.VERCEL_REGION ?? null,
    branche: env.VERCEL_GIT_COMMIT_REF ?? null,
  };
}
