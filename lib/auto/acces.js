...
export function descriptionEnvironnement(env = {}) {
  const adresse = String(env.NEXT_PUBLIC_SUPABASE_URL || "");
  const projet = adresse.match(/^https:\/\/([a-z0-9]{20})\.supabase\.co/)?.[1] ?? null;
  const base = projet === PROJET_PRODUCTION ? "production" : projet ? "autre" : "inconnue";
  return {
