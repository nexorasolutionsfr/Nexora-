...
// l'apprenait qu'APRÈS avoir envoyé le fichier (constat du 17 sept. 2026).
//
// Une troisième limite manquait, et c'est la plus fréquente : la lecture
// gratuite extrait le TEXTE contenu dans le PDF (lib/auto/lecture/texte-pdf.js).
// Une facture scannée ou photographiée est une image : elle est conservée, pas
// lue. « Une facture PDF est lue automatiquement » promettait donc plus que ce
// qui se produit.
export function phraseLimites({ lisibles = [], tailleLectureMax = null, pagesMax = null } = {}) {
  ...
  const image = lisibles.some((format) => format.startsWith("image/"));
  const pages = pagesMax ? `, ${pagesMax} pages au plus` : "";
  if (image) return `${depot} Nexora lit les documents de moins de ${mo(tailleLectureMax)}${pages} ; au-delà, le document est conservé et vous renseignez les informations.`;
  return `${depot} Nexora lit les PDF contenant du texte, de moins de ${mo(tailleLectureMax)}${pages} ; un scan, une photo ou un PDF plus lourd est conservé, et vous renseignez les informations.`;
}
