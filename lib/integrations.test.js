...
test("aucun appel sortant ne contourne l'interrupteur", async () => {
  ...
  const SORTANT = /api\.resend\.com|api\.stripe\.com|googleapis\.com|api\.anthropic\.com|STRIPE_SECRET_KEY|RESEND_API_KEY|GOOGLE_CLIENT_SECRET|ANTHROPIC_API_KEY/;
  const GARDE = /integrationsSortantes|configurationLecture/;
  // Le client Anthropic ne sert que par la route de lecture, qui ne le crée
  // que si configurationLecture() l'a rendu disponible.
  const EXCEPTIONS = new Set(["lib/auto/lecture/anthropic.js"]);
  const fichiers = [];
  ...
  assert.ok(sortants.length >= 7, `inventaire trop court : ${sortants.join(", ")}`);
