...
function DevisView({ devisList, onAccept, onRefuse, onUpdateMontant }) {
  if (devisList.length === 0) {
    return <EmptyState icon={ReceiptText} title="Aucun devis en attente" subtitle="Les demandes de devis apparaîtront ici, prêtes à valider ou ajuster." />;
  }
  return (
    <div className="space-y-4">
      {devisList.map((d) => (
...
function imprimerFacture(facture, garageData) {
  ...
  const html = `
    ...
    <body>
      ...
      <table>
        <tr><th>Description</th><th>Montant HT</th><th>Montant TTC</th></tr>
        <tr><td>Prestation atelier</td><td>${Number(facture.montant_ht || 0).toFixed(2)} €</td><td>${Number(facture.montant_ttc || 0).toFixed(2)} €</td></tr>
      </table>
      <div class="total">Total à payer : ${Number(facture.montant_ttc || 0).toFixed(2)} € TTC</div>
    </body>
    </html>
  `;
...
function FacturesView({ rendezVous, factures, prestations, garageData, onGenerer, onMarquerPayee }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <div className="font-semibold text-slate-900 text-[15px] mb-3">Factures</div>
        {factures.length === 0 ? (
            ...
            {factures.map((f) => (
              <div key={f.id} className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm flex items-center justify-between flex-wrap gap-3">
                <div>
                  ...
                  <div className="text-[12.5px] text-slate-500">{f.clients?.nom} · {Number(f.montant_ttc || 0).toFixed(2)} € TTC</div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => imprimerFacture(f, garageData)} className="text-sm font-medium px-3.5 py-2 rounded-xl border border-slate-200 text-slate-600">
                    Imprimer / PDF
                  </button>
                  {f.statut !== "payee" && (
...
if (updateError) {
  ...
  const handleGenererFacture = async (rdv) => {
    ...
    const montantHt = Number(prestation?.prix_ht || 0);
    const montantTtc = Math.round(montantHt * 1.2 * 100) / 100;
...
/>
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="p-5 md:p-8">
          ...
          {view === "devis" && <DevisView devisList={devisList} onAccept={handleAcceptDevis} onRefuse={handleRefuseDevis} onUpdateMontant={handleUpdateDevisMontant} />}
          {view === "verifier" && <ErreursView erreurs={erreurs} onResoudre={handleResoudreErreur} />}
          {view === "factures" && <FacturesView rendezVous={rendezVous} factures={factures} prestations={prestations} garageData={garageData} onGenerer={handleGenererFacture} onMarquerPayee={handleMarquerFacturePayee} />}
