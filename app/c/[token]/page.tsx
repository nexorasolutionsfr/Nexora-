...
export default function ConfirmationTokenPage({ params }) {
  ...
  useEffect(() => {
    async function load() {
      const { data, error } = await supabase.rpc("lire_confirmation_par_jeton", { p_token: token });
      if (error || !data || !data.length) {
        setError("Ce lien n'est plus valable. Contactez votre garage pour obtenir un nouveau lien.");
      } else {
        setInfo(data[0]);
  ...
  async function repondre(choix) {
    setSaving(true);
    const { data, error } = await supabase.rpc("repondre_confirmation_par_jeton", { p_token: token, p_reponse: choix });
    setSaving(false);
    if (error || data !== true) {
      setError("Ce lien n'est plus valable. Contactez votre garage pour obtenir un nouveau lien.");
      return;
